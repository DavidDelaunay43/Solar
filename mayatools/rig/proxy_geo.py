...
def create_proxy_geo(jnts: list, subdiv_axis: int = 8, subdiv_height: int = 2) -> None:
    ...
    for jnt, next_jnt in zip(jnts, next_jnts):
        ...
        cmds.polyCylinder(name=geo, height=radius, subdivisionsAxis=subdiv_axis, subdivisionsHeight=subdiv_height, constructionHistory=False)
        cmds.matchTransform(geo, jnt, position=True)
        cmds.parent(geo, jnt)
